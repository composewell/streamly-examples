...
import Streamly.Internal.Data.Fold (Fold(..), Step(..), foldt')
...
{-# NOINLINE foldDateTimeAp #-}
foldDateTimeAp :: Array Char -> IO Int
foldDateTimeAp arr =
    let t =
                mkTime
            <$> decimal 4  -- year
...
foldBreakDateTime :: Array Char -> IO Int
foldBreakDateTime arr = do
...
parseBreakDateTime :: Array Char -> IO Int
parseBreakDateTime arr = do
    let s = Stream.unfold Array.reader arr
...
timeBench =
    ...
    in bgroup "parseDateTime"
        ...
        -- , bench "fold applicative" $ nfIO $ foldDateTimeAp arr
        -- , bench "foldBreak" $ nfIO $ foldBreakDateTime arr
        -- , bench "parseBreak" $ nfIO $ parseBreakDateTime arr
        ]
